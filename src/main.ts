...
function workspaceExtras() {
  ...
  const recovery = recoveryNotice ? `<section class="privacy-status recovery-notice" role="alert"><b>数据库恢复提示</b><span>${escapeHtml(recoveryNotice)}</span><small>应用已启动新数据库；旧数据仍保留在上述恢复目录中。</small></section>` : '';
