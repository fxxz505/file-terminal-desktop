...
type View = 'workspace' | 'search' | 'assistant' | 'conversations' | 'diagnostics' | 'tasks' | 'settings' | 'about';
...
function diagnosticsPage() {
  ...
  return `<section class="single-panel panel diagnostics-page"><header><div><small>INDEX DIAGNOSTICS</small><h2>索引诊断</h2><span>仅查看本机索引元数据，不会读取或导出正文。</span></div><div><button class="quiet" id="refresh-diagnostics">刷新</button><button class="quiet" id="retry-failed-diagnostics" ${failures.length ? '' : 'disabled'}>重试失败项 (${failures.length})</button></div></header><div class="diagnostic-filters"><select id="diagnostic-status"><option value="">全部状态</option><option value="failed">提取失败</option><option value="skipped">已跳过</option><option value="indexed">已索引</option><option value="pending">等待处理</option></select><button class="quiet" id="export-diagnostics">导出安全摘要</button></div>${diagnosticStatus ? `<p class="form-warning">${escapeHtml(diagnosticStatus)}</p>` : ''}<div class="diagnostic-list">${rows || '<p class="result-placeholder">暂无诊断记录。接入资料夹后会显示每个文件的索引状态。</p>'}</div></section>`;
}
...
function tasksPage() {
  ...
  return `<section class="single-panel panel tasks-page"><header><div><small>BACKGROUND TASKS</small><h2>后台任务</h2><span>显示真实持久化索引、媒体和下载任务；只有支持的操作才会显示按钮。</span></div><button class="quiet" id="refresh-background-tasks">刷新</button></header><div class="task-list" data-testid="task-list">${rows || '<p class="result-placeholder">当前没有等待、运行或失败的后台任务。</p>'}</div></section>`;
}
...
function render() {
  ...
  const pages: Record<View, { title: string; subtitle: string; content: string }> = {
    ...
    tasks: { title: '后台任务', subtitle: '索引与媒体任务的真实状态和可用操作', content: tasksPage() },
...
async function fetchCloudModels() { syncCloudDraftFromDom(); const draft = { ...cloudDraft }; if (!draft.providerId || !draft.displayName || !draft.baseUrl || !draft.apiKey && !cloudConfig?.configured) { formWarning = { target: 'cloud', message: '请先填写名称、标识、基础地址和 API Key。' }; render(); return; } isWorking = true; error = ''; formWarning = null; render(); try { cloudConfig = await invoke<CloudProviderConfig>('save_cloud_provider_config', { input: draft }); cloudDraft = { ...draft, apiKey: '' }; cloudProviders = await invoke<CloudProviderConfig[]>('list_cloud_providers'); cloudModels = await invoke<CloudModel[]>('fetch_cloud_models', { input: { providerId: draft.providerId } }); if (!cloudDraft.model && cloudModels[0]) cloudDraft.model = cloudModels[0].id; } catch (reason) { formWarning = { target: 'cloud', message: String(reason) }; } finally { isWorking = false; render(); } }
...
function bind() {
  ...
  document.querySelectorAll<HTMLElement>('[data-view]').forEach(button => button.addEventListener('click', () => { activeView = button.dataset.view as View; if (activeView === 'diagnostics') refreshDiagnostics().catch(reason => { diagnosticStatus = String(reason); render(); }); else if (activeView === 'tasks') refreshBackgroundTasks().catch(reason => { error = String(reason); render(); }); else render(); }));
  ...
  document.querySelector('#refresh-diagnostics')?.addEventListener('click', () => refreshDiagnostics().catch(reason => { diagnosticStatus = String(reason); render(); }));
  ...
  document.querySelector('#refresh-background-tasks')?.addEventListener('click', () => refreshBackgroundTasks().catch(reason => { error = String(reason); render(); }));
