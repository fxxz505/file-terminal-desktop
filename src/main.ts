...
function assistantPanel() {
  ...
  const assistantHistory = `<div class="assistant-history-brand"><span>${icons.spark}</span><div><b>AI 协作</b><small>LOCAL / CLOUD</small></div></div><button class="assistant-history-new" data-new-conversation="true">${icons.spark}<span>新建对话</span></button><div class="assistant-history-section"><small>对话</small><span>${conversations.length} 段</span></div><div class="assistant-history-list">${history}</div><div class="assistant-history-foot"><span class="online-dot"></span><span>对话仅保存在本机</span></div>`;
